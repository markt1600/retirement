...
year timing, 0.8%/yr fees (the page default).
"""
import os
import sys
...
def main():
    out = ["# Trinity across markets (full history, inflation-adjusted withdrawals)", "",
           ...
           f"year, fees {FEES}%/yr.  Local-currency returns, local bonds and local CPI for Japan/UK/Germany; USD, US "
    ...
    for alloc in (75, 50):
        for rate in (4, 3.5, 3):
            ...
            for p in PERIODS:
                ...
                for m in MARKETS:
                    y = series[m][0]
                    rows = T.success_table(series[m], y[0], y[-1], [p], [rate], [alloc], True, "start", m, fees_pct=FEES)
    ...
    for thr in (100, 95):
        ...
        for p in PERIODS:
            ...
            for m in MARKETS:
                y = series[m][0]
                r = T.max_safe_rates(series[m], y[0], y[-1], p, 75, True, "start", (thr,), fees_pct=FEES)[thr]
